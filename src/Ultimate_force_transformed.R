...
model_ultforce_full<- lm(log(U)~ E+G+H+I+J+K+M+log(Q),data=dataset_2)
...
theme_linedraw()+
  ...
  scale_x_continuous(breaks=seq(3,5.5,0.5),limits=c(3, 5.5),expand = c(0, 0.2))+
  scale_y_continuous(breaks=seq(-1,1,0.5),limits=c(-1, 1),expand = c(0, 0.2))


ggsave("reg_model_ultforce_full_transformed.png", plot = last_plot(), device = "png", 
...
ggsave("qq_model_ultforce_full_transformed.png", plot = last_plot(), device = "png", 
       path = "C:\\Users\\J\\Desktop\\Course_DataAnalysis\\Proposal\\Pictures",
       scale = 1, width = 6, height = 6, units ="cm",
       dpi = 300)
...
model_ultforce_reduced<- lm(log(U)~I+J+K+M+log(Q),data=dataset_2)
...
model_ultforce_full_stepwise<- lm(formula = U ~ G + Q + J + E + M, data = dataset_2)
...
## res plot
res=c(resid(model_ultforce_full_stepwise))
fitted_values=c(model_ultforce_full_stepwise[["fitted.values"]])
modeldata_ultforce_reduced=data.frame(res,fitted_values)
...
theme_linedraw()+
  ...
  theme(axis.text.x = element_text(family="Times New Roman",
        ...
        axis.title.y = element_text(family="Times New Roman", colour="black", size=9),
        plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm"))+
  scale_x_continuous(breaks=c(3,3.5,4,4.5,5,5.5,6),limits=c(3, 6),expand = c(0, 0.2))+
  scale_y_continuous(breaks=seq(-1,1,0.5),limits=c(-1, 1),expand = c(0, 0.2))

ggsave("reg_model_ultforce_transformed.png", plot = last_plot(), device = "png", 
       path = "C:\\Users\\J\\Desktop\\Course_DataAnalysis\\Proposal\\Pictures",
       scale = 1, width = 6, height = 6, units ="cm",
       dpi = 300)
...
res <-c(rstandard(model_ultforce_full_stepwise))
fitted_values <-c(model_ultforce_full_stepwise[["fitted.values"]])
modeldata_ultforce_reduced <-data.frame(res,fitted_values)
stdresiduals <- rstandard(model_ultforce_full_stepwise)
...
ggsave("qq_model_ultforce_final.png", plot = last_plot(), device = "png", 
       path = "C:\\Users\\J\\Desktop\\Course_DataAnalysis\\Proposal\\Pictures",
       scale = 1, width = 6, height = 6, units ="cm",
       dpi = 300)